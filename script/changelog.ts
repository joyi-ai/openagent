...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/opencode packages/sdk packages/plugin packages/desktop packages/app packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/opencode/src/cli/cmd/")) areas.add("core")
...
function getSection(areas: Set<string>): string {
  ...
  const priority = ["core", "app", "tauri", "sdk", "plugin", "extensions/zed", "github"]
...
export async function generateChangelog(commits: Commit[], opencode: Awaited<ReturnType<typeof createOpencode>>) {
  ...
  const sectionOrder = ["Core", "Desktop", "SDK", "Extensions"]
  const lines: string[] = []
